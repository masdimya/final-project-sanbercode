		...
		<div class="card-body">
			...
			<div class="card">
				...
				<div class="card-footer d-flex">
					...
					@if ($answer->is_answer)
						<p class="ml-2 pt-2 mb-0 text-success"><i class="fa fa-check-circle" aria-hidden="true"></i> Approved</p>
					@endif

					@if (! $answer->is_answer && $question->user->id == Auth::id())
						<form action="{{route('answer.approve',['answer_id'=>$answer->id])}}" method="post">
							@csrf
							<button class="btn btn-success" > <i class="fa fa-check" aria-hidden="true"></i> Approve Answer</button>
						</form>
					@endif
				</div>
			</div>
